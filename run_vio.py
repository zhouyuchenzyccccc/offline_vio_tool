...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--visualize", action="store_true", help="Enable ORB-SLAM3 native viewer (same as --use_viewer)")
    p.add_argument("--vis_with_pointcloud", action="store_true", help="Deprecated: ignored when using ORB-SLAM3 native viewer")
    p.add_argument("--vis_keyframes_only", action="store_true", help="Deprecated: ignored when using ORB-SLAM3 native viewer")
...
def main() -> None:
    ...
    # Reuse ORB-SLAM3 native viewer instead of custom Open3D visualizer.
    if args.visualize:
        args.use_viewer = True

    if (args.vis_with_pointcloud or args.vis_keyframes_only) and args.visualize:
        print("Note: --vis_with_pointcloud/--vis_keyframes_only are ignored. Using ORB-SLAM3 native viewer only.")
    ...
    if args.visualize and not args.run_slam:
        print("Note: ORB-SLAM3 native viewer is only available during --run_slam. Existing --traj_ws will not open ORB viewer.")
